...
export class StarWarsFPS implements BasicApp {
    ...
    private walTextCoords: Float32Array;
    ...
    private async init() {        
        ...
        this.cam.setPosition(glm.vec3.fromValues(0, 5, 90));
    ...
    private async initQuadBuffers() {
        ...
        this.walTextCoords = new Float32Array([
        //  u  v  
            -33.3, -50,   // bottom-left
        ...
        this.gl.bufferData(this.gl.ARRAY_BUFFER, this.walTextCoords, this.gl.STATIC_DRAW);
        this.gl.vertexAttribPointer(
    ...
    async initModels() {
        ...
        this.stormtrooper2_Model.position =glm.vec3.fromValues(20, 0, 27);
        this.stormtrooper2_Model.scaleUniform(1.4);
        this.stormtrooper2_Model.rotate(0,0,-20);
    ...
    private onKeyDown(e: KeyboardEvent) {
        ...
        switch (e.key.toLocaleLowerCase()) {
            case 'a': // left                
                this.cam.move(glm.vec3.scale(displacement, this.cam.right, - this.deltaTime * this.camMoveSpeed), this.elementsPositions, this.elementsBoxes);
                break;
            case 'd': // right
                this.cam.move(glm.vec3.scale(displacement, this.cam.right, this.deltaTime * this.camMoveSpeed), this.elementsPositions, this.elementsBoxes);
                break;
            case 'w': // forward                
                this.cam.move(glm.vec3.scale(displacement, this.cam.direction, this.deltaTime * this.camMoveSpeed), this.elementsPositions, this.elementsBoxes);
                break;            
            case 's': // backward
                this.cam.move(glm.vec3.scale(displacement, this.cam.direction, - this.deltaTime * this.camMoveSpeed), this.elementsPositions, this.elementsBoxes);
                break;
